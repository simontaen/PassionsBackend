    ...
    fetchArtist: function(parseArtist) {
      ...
      return wrappedHttpRequest(apiUrl + endpoint, params, "spotify.fetchArtist").then(function(httpResponse) {
        ...
        if (_.isEmpty(exactMatches)) {
          console.log("INFO (spotify): No exact match found for Artist " + artistName + " out of " + _.size(httpResponse.data.artists.items) + ".");
          ...
          if (_.size(exactMatches) > 1) {
            ...
            console.log("INFO (spotify): Found " + _.size(exactMatches) + " exact matches for Artist " + artistName + " on iTunes.");
          }
